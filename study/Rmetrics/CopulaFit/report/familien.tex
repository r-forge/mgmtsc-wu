...
erf\"ullen. F\"ur statistische und ökonometrische Zwecke ist es daher
durchaus dienlich, eine Reihe von verschiedenen Copulae zur
Verf\"ugung zu haben. Man unterscheidet vor allem zwei große Gruppen
von Copula-Familien, nämlich die archimedischen Copulae und die
...
\emph{Gauß'sche Copula} (oder Normal Copula) und die \emph{t-Copula}
...
Die Gauß'sche Copula (Normal-Copula) ist die Copula der multivariaten
...
darstellt. Dann ist die Gauß'sche Copula definiert mit
...
Ränder sind also univariat t-verteilt, dh: $(u_1, \ldots, u_n) =
...
\begin{itemize}
\item Sie können sehr einfach erzeugt werden.
\item Es gibt viele parametrische Familien von Copulae, die ebenfalls
  zu den archimedischen Copulae gehören.
\item Archimedische Copulae besitzen viele gew\"unschte Eigenschaften.
\end{itemize}

So vielfältig diese Klasse ist, so unterschiedlich ist auch die
Abhängigkeitsstruktur, die erzeugt werden
...
Parameter) gegeben wird. Häufig verwendet werden von diesen vor allem
...
Erf\"ullt $\varphi$ die Bedingung der Konvexität, so gilt nun f\"ur
