...
Für unsere Berechnungen wird immer eine numerische
Optimierungsfunktion benötigt,  welche die Zielfunktion optimiert. In
diesem Zusammenhang werden die in \emph{R} implementierten Funktionen genutzt. 
Ein Problem bei der Optimierung liegt darin, dass es viele
Kombinationen von Randverteilungen und Copulaes gibt. Es wird also die optimale Lösung der Kombination von Copulae und Randverteilung suchen.
...
Bei der EML-Methode wird versucht, sowohl die Randverteilungen als auch die Copulae gleichzeitig über einen MLE zu optimieren.
Hierbei wird eine Datenmatrix $ \chi = \left\{x_{1t},x_{2t},...,x_{nt}\right\}^{T}_{t=1} $ benötigt. Die zu optimierende Funktion lauted dann:
...
Vielzahl von Randverteilungen und Copulae her, so erhälten man durch 
...
verwenden wir noch andere Methoden wie beispielsweise die
...
Schätzen der richtigen Copulae und anderseits aus dem Schätzen der
...
geschätzt. Im zweiten Schritt wird mit den angepassten Randverteilungen die Copulae nach
...
Bei der vorhergehenden Methode haben wir die benötigte
Verteilungsfunktion der Randverteilung geschätzt. Diesen Schritt kann
man bei der CML-Methode weglassen, indem die empirische
Verteilungsfunktion ($\hat{F_i}(x_{it})$ mit $i = 1,....,n$) verwendet
wird. Diese wurde folgendermaßen definiert:
